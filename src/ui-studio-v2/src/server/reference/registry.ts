...
export const registryHits: RegistryHit[] = [
  ...
  },
  {
    ...
    kind: "workflow",
    name: "Order Approval",
    description: "Draft to Submitted to Approved or Rejected",
  },
  {
    ref: "connector.erp",
    kind: "connector",
...
export const typeShapes: Map<string, TypeShape> = new Map([
  ...
  [
    "entity.OrderLine",
    {
      ...
      fields: [
        ...
        { name: "partNumber", type: "string", required: true, description: "Part or SKU reference" },
  ...
  [
    "rule.maxDiscount",
    {
      ref: "rule.maxDiscount",
      fields: [
        { name: "maxAllowed", type: "number", required: true, description: "Max discount percent for role" },
        { name: "result", type: "boolean", required: true, description: "Rule evaluation result" },
      ],
    },
  ],
...
export const allRegistryKeys: ReadonlySet<string> = new Set(registryHits.map((hit) => hit.ref))
