...
export interface SeedAppRecord {
  id: string
  name: string
  vertical: string
  description: string
...
const navDms: NavigationNode = {
  ...
  entries: [
    ...
    { logicalKey: "nav.newOrder", label: "New Order", pageRef: "page.salesOrder", icon: "plus" },
  ],
}
...
const toyotaHeaderOverride: NodeBase = {
  ...base("uuid-toyota-003", "cmp.objectHeader.toyota", "tenant", {
    ...
    overrideOps: [{ op: "set", path: "props.subtitle", value: "Toyota Sales Order" }],
  }),
}
...
const dealerXThemeOverride: NodeBase = {
  ...base("uuid-dealerx-001", "theme.automotive.dealerx", "org", {
    overrideOf: "theme.automotive",
    overrideOps: [{ op: "merge", path: "tokens", value: { colorAccent: "#f59e0b" } }],
  }),
}
...
const dealerXNavOverride: NodeBase = {
  ...base("uuid-dealerx-002", "nav.dms.dealerx", "org", {
    overrideOf: "nav.dms",
    overrideOps: [{ op: "insert", path: "entries", logicalKey: "nav.dealerDashboard" }],
  }),
}
...
export const seedApp: SeedAppRecord = {
  id: "app.dms",
