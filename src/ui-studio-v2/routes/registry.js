...
const registryHits = [
  { ref: "entity.SalesOrder", kind: "entity", name: "Sales Order", description: "Top-level dealership sales order" },
  { ref: "entity.OrderLine", kind: "entity", name: "Order Line", description: "Line item on a sales order" },
  { ref: "entity.Customer", kind: "entity", name: "Customer", description: "Dealership customer" },
  { ref: "rule.orderHasLines", kind: "rule", name: "Order Has Lines", description: "Validates that an order has at least one line item" },
  { ref: "rule.maxDiscount", kind: "rule", name: "Max Discount", description: "Enforces maximum discount percentage per role" },
  { ref: "wf.orderApproval", kind: "workflow", name: "Order Approval", description: "Draft → Submitted → Approved / Rejected" },
  { ref: "connector.erp", kind: "connector", name: "ERP Connector", description: "Outbound integration to enterprise ERP system" },
]
...
// GET /api/v1/registry/shape/:ref
router.get("/shape/:ref", (req, res) => {
  const shape = typeShapes[req.params.ref]
  if (!shape) {
    return res.status(404).json({ message: `Shape not found: ${req.params.ref}` })
  }
  res.json(shape)
})
