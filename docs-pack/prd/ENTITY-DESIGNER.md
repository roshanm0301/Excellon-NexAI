# PRD: ENTITY-DESIGNER.md â€” Entity Designer & Backend Compiler
...
- **Row actions:** Edit â†’ `/admin/entities/{id}/edit`, Delete (confirm dialog), Duplicate (copies payload, opens new editor)
- **Header actions:** New Entity, Bulk Delete, View Map (`/admin/entities/map`)
- **State:** TanStack Query â€” invalidate on delete or publish
...
- **State:** All local `useState` hooks â€” no global store
- **Load (edit mode):** `GET /api/v1/artifacts/{id}/latest` â†’ parse payload into tab state variables
...
- **Fetches:** All entity schema artifacts â†’ creates nodes + edges from `reference` fields and `has_many`/`belongs_to` relationships
...
- **Interactions:** Click node â†’ detail panel; Double-click â†’ editor; New Entity modal; PNG export

---

## Frontend â€” 8 Tabs

### Tab 1 â€” Schema (FieldBuilder)

- **File:** `src/react/src/components/studio/EntityDesigner/FieldBuilder.tsx`
- System fields section (locked): `id` (gear â†’ IDConfigPanel), `created_at`, `updated_at`, `created_by`, `updated_by`
- User fields: accordion rows, each with sections:
  - **Core:** name (snake_case), label, type (FieldTypeSelector), required toggle
  - **Storage:** `physical` | `computed` â€” when computed, ExpressionEditor replaces value input
...
- Example: prefix=`ORD`, separator=`-`, padding=6 â†’ `ORD-000001`
...
### Tab 3 â€” Relationships (RelationshipBuilder)
...
### Tab 4 â€” Virtual Entity (CapabilityFlagsPanel)
...
### Tab 6 â€” Node Scope (NodeScopePicker)
...
### Tab 8 â€” Data Retention (RetentionPanel)
...
- Visual pipeline diagram showing stages for selected mode:
  ...
  - GDPR: Recycle Bin â†’ Archived â†’ Anonymised â†’ Pending Purge â†’ Hard Delete (timing multipliers: 1Ã—, 2Ã—, 3Ã—)

---

## Frontend â€” NLP Assistant Panel

- Opens from sparkles (âœ¨) icon in editor header
...
- **AI Import:** Paste plain-text description â†’ `POST /api/nlp/import` â†’ generates initial field list

---

## Frontend â€” Component Inventory
...
## Backend â€” Artifact Versioning API

All artifact endpoints are in `src/go/internal/admin/handler.go`.

| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/api/v1/artifacts` | List artifacts â€” filter by `artifact_type`, `tenant_id` |
...
## Backend â€” 6-Step Compiler Pipeline

**File:** `src/go/internal/compiler/entity_schema.go`
**Input:** `overlay.MergedArtifact` (after 5-layer overlay merge)
**Output:** `compiled_artifact.payload` â€” what all runtime code reads

### Step 1 â€” Load Field Type Defaults
- Queries active `field_type_catalog` artifact
- Builds map: `customTypeName â†’ { display_mask, null_text, validation, grid_hints }`
- Non-fatal if catalog missing â€” map is empty, step skipped

### Step 2 â€” Process User Fields
- Skip any field with `_removed: true` (overlay REMOVE operation)
- 2a: Normalise type key â€” `"field_type"` â†’ `"type"`
- 2b: Reference field defaults â€” missing `display_field` â†’ `"name"`, missing `value_field` â†’ `"id"`
- 2c: Picklist resolution â€” if `lookup_type = "picklist"`: load picklist artifact â†’ inline items OR write datasource binding fields
- 2d: Field type defaults â€” merge custom type defaults for missing properties (never overwrite explicit values)
- 2e: Normalize legacy `compute_mode` â€” delete `compute_mode: "template"` or `"javascript"`. Only JSONata `compute_expression` is valid.

### Step 3 â€” Inject System Fields
Always appended after user fields:
- `id` (uuid) â€” carries `id_strategy` from `id_config`
- `created_at` (datetime) â€” auto-set on insert
- `updated_at` (datetime) â€” auto-set on update
- `created_by` (string) â€” from request context
- `updated_by` (string) â€” from request context
- `display_id` (string) â€” only when `display_id.enabled = true`

User fields with same name as system fields are **overwritten** by system fields.

### Step 4 â€” ID Config, Display ID, Composite Keys
- ID strategy: reads `id_config.strategy` â†’ validates `uuid_v4` | `uuid_v7` â†’ stamps at schema top level and on `id` field
- Display ID: when `enabled = true` â†’ injects field + carries `display_id_config` into compiled output
- Composite keys: validates `name` (snake_case â‰¤50 chars), fields exist â†’ carries into compiled schema â†’ calls `indexmgmt.QueueIndexes()`

### Step 5 â€” Pass-through Fields
Carried unchanged into compiled schema:

### Step 6 â€” Metadata Stamp
Sets `_compiled_at` to current UTC timestamp.

---

## Backend â€” Entity Runtime CRUD
...
3. Rules evaluation: `rulesEval.EvaluateRules()` â†’ if blocked â†’ HTTP 422 with violations
4. ID generation: `idgen.Generate(idStrategy)` â€” uuid_v7 or uuid_v4
5. Display ID: atomic upsert into `entity_sequence` â†’ format as `{prefix}{sep}{zero-padded}`
