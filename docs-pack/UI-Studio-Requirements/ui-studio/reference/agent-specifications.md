# Agent Specifications â€” All 17 Agents
...
Reuse artifact_header and artifact_version tables â€” do NOT rebuild them.
...
Requirements (apply to all phases):
  - All DB queries filter by tenant_id â€” never expose cross-tenant data
...
When assigned to a phase, read that phase file first:
  Phase 1 â†’ P1-metadata-foundation.md, section 1.2
  Phase 2 â†’ P2-component-registry.md
  Phase 3 â†’ P3-view-designer.md
  Phase 4 â†’ P4-runtime-renderer.md
  Phase 5 â†’ P5-event-engine.md
  Phase 6 â†’ P6-header-line-workspace.md
  Phase 8 â†’ P8-publish-governance.md
  Phase 9 â†’ P9-role-variants.md
  Phase 10 â†’ P10-ai-templates.md
```
...
EXTEND existing files â€” do NOT rebuild from scratch.
...
Requirements (apply to all phases):
  ...
  - Use shadcn/ui components â€” do not add new component libraries
...
Requirements:
  - NEVER load draft versions â€” only artifact_version rows where is_active=true
  - Broken component does NOT crash the page â€” ComponentErrorBoundary per component
...
Tasks:
  1. Write SQL seed file app/db/seed/008_ui_studio_components.sql
     â€” All 56 platform components exactly as specified in section 2.1
  2. Build ComponentRegistryPage.tsx per section 2.2
     â€” Category filter chips, surface filter, row expand for config_schema, plugin section
  3. Add navigation routes per section 2.3

The seed SQL is the source of truth for component metadata.
Do NOT hardcode component lists in TypeScript â€” always query from the DB.
```
...
Requirements by phase:
  Phase 3 (M4): Binding tab in Right Inspector, EntityFieldPicker.tsx, DataSourcePanel.tsx
    â€” Use InspectorProps interface agreed with Agent 5 before starting
  Phase 4 (M5): BindingResolver.ts, PermissionFilter.ts
    â€” Safe expression evaluation: no new Function(), no eval()
    â€” Computed bindings use JSONata library
  Phase 6 (M6): Line-level binding resolver, dynamic column injection in LineGridRuntime
  Phase 9 (M9): Cascading lookup â€” parent field change triggers re-query with new filter param
    â€” Changing parent field value MUST clear the child field value before re-querying
```
...
Requirements:
  - eventEngine.ts MUST be pure TypeScript â€” zero React imports â€” independently testable
  ...
  - Expression evaluation: use JSONata â€” no eval(), no new Function()
...
Specific behavior:
  - Allowed actions rendered as enabled buttons (from API response â€” not hardcoded)
...
GENERIC RULE (hard rule):
  ...
  The Sale Order E2E test is the exit condition â€” it must pass through generic config.

Specific requirements:
  - ViewCode propagated on ALL API calls (query param or request body)
  - Totals panel uses JSONata expressions â€” no eval(), no new Function()
  - Save persists header + all lines in a single DB transaction â€” partial save must roll back
...
CRITICAL SECURITY RULES:
  1. Hidden fields MUST be ABSENT from the DOM â€” NOT CSS display:none or visibility:hidden
     Test: document.querySelector('[data-field="salary"]') === null
  2. Masked fields show *** â€” not the real value
  3. Tenant isolation: all permission evaluations include tenant_id in context

Role variant implementation:
  - Variants are OVERLAYS over the base published payload â€” not clones
  - Multiple matching variants: highest priority (lowest priority number) wins
  - Variants stored in ui_view_variant table â€” fetched at runtime per user context
```
...
Requirements:
  ...
  - PreviewModal renders StudioRenderer in sandboxed iframe â€” not in main window
...
Test file naming conventions:
  *.test.ts                    â€” unit tests (Vitest)
  *.integration.test.ts        â€” integration tests (real PostgreSQL via testcontainers)
  *.e2e.ts                     â€” Playwright E2E tests
...
**Phases:** M2â€“M11 (milestone summaries) + M11 (complete doc suite)
...
Include code examples from actual implemented code â€” not hypothetical examples.
```
...
For the phase specified:
  ...
  2. Verify the milestone Gate Condition is FULLY met (from the phase file âœ… Gate section)
