# UI Guidelines & Project Principles â€” UI Studio
...
It allows authorized business configurators to create, configure, and publish fully functional business views â€” including complex ERP transaction documents â€” without writing code. Every view is configuration, not code. Every configuration choice is governed by the system â€” not left open-ended.
...
| Entity schema and field definitions | **Entity Designer** | UI Studio reads â€” never writes |
| Business validation truth | **Rule Engine** | UI Studio displays results â€” never computes |
| Security and RBAC decisions | **Permission Engine** | UI Studio applies decisions â€” never makes them |
...
These principles apply to every screen in UI Studio â€” both the designer and the runtime renderer.
...
|---|---|---|---|
| M1 | P0 | Gap analysis â€” no code changes | Every P0/P1/P2 feature classified |
...
| M4 | P3 | View designer shell â€” create, drag, save draft | Designer produces valid draft payload |
| M5 | P4 | Runtime renderer â€” views render with real data | Published view renders with real entity data |
...
1. **P1 (Metadata Foundation)** first â€” the DB schema is the contract all other phases build on
2. **P2 (Component Registry)** before P3 â€” the designer can only place components that are registered
3. **P3 (Designer)** before P4 â€” the runtime renders what the designer produces
4. **P4 (Runtime Renderer)** before P5 â€” events need a rendered form to operate on
5. **P5 (Event Engine)** before P6 â€” transaction workspace uses the event engine for recalculation
7. **P8 (Governance)** after P6/P7 â€” publish/rollback needs working surfaces to govern
8. **P9 (Role Variants)** before P10 â€” AI generation should produce variant-aware views
9. **P10 (AI + Templates)** last â€” builds on all previous surfaces and features
...
User navigates to a transaction screen
  â†’ UI resolves ViewCode for this route (e.g., SO_VEHICLE_BOOKING)
  â†’ ViewCode included in every API call to backend
  â†’ Backend Rule Engine selects rule set for this ViewCode
  â†’ UI Studio loads the published view for this ViewCode
```
...
| Hidden field found in page source | Security exposure | Permission filter removes from DOM â€” not CSS hidden |
...
**For each phase:**
1. Open `docs/business_process/phases/P{N}-*.md` â€” this is the business requirement
2. Open `docs/ui-studio/phases/P{N}-*.md` â€” this is the implementation specification
...
1. Open `docs/business_process/reference/feature-matrix.md` â€” lists all 63 features
2. Open `docs/ui-studio/reference/feature-matrix.md` â€” lists implementation status
