# P10 — Template Gallery, AI Generation & Analytics

**Milestone:** M11
**Track:** Track 11 — AI-Assisted View Generation + §7 P2 Features
...
Allow business configurators to start from a template or describe a view in plain language, and get a fully structured draft view in seconds. Reduce the time to create a new view from hours to minutes. All AI output must land as a draft — never auto-published — so a human always reviews before it goes live.
...
|---|---|---|---|
| 1 | Standard CRUD — Master Data | standard_crud | Any entity that needs a basic form + list |
| 2 | Master with Related Records | advanced_crud | Entity with one or more related entity grids |
| 3 | Transaction — Header + Lines | header_line | Purchase Order, Sale Order, Service Job |
| 4 | Approval Workflow Form | standard_crud | Any entity with an approval workflow |
| 5 | Dashboard — KPI + Chart | dashboard | Operational overview for a domain |
| 6 | Kanban Board | kanban | Any entity with a status field |
| 7 | Wizard — Multi-Step Entry | wizard | Complex data entry split into steps |
| 8 | Inspection / Checklist | standard_crud | Quality inspection, service checklist |
| 9 | Report View — Read Only | detail_page | Printable record summary |
...
|---|---|
| Output always DRAFT | Human must review before publishing — AI cannot publish directly |
...
| Invalid entity → clear error | If AI references an entity that does not exist, return a clear error message |
...
| P2-53 Accessibility Checks | Pre-publish linter ensures inputs have labels (A001–A005 rules) |
| P2-54 Localization Checks | Pre-publish linter flags hardcoded text that should be translatable (L001–L004) |
...
| P2-57 AI View Generation | Describe a view in natural language → get a draft |
...
- Clone template → creates draft with full layout scaffold.
...
- Run through publish validation — auto-fix duplicate keys and invalid component codes.
- Return as DRAFT — never auto-publish.
...
analytics telemetry, performance alerts, accessibility linter (A001–A005),
localization linter (L001–L004), advanced expression editor, no-code rule wizard,
...
- AI output is always draft — configurator must review and manually publish
...
- [ ] AI generation: valid description → draft ViewArtifactPayload created
- [ ] AI output returned with DRAFT status — cannot be published directly from NlpPanel
- [ ] AI generation logged with action='ai_generated' in audit trail
- [ ] AI draft shows "AI Generated" badge in designer
- [ ] Invalid entity type in prompt → clear 422 error message
...
- [ ] Export view → valid JSON bundle downloadable
- [ ] Import JSON bundle → view appears as DRAFT
- [ ] Import with missing plugin component → warning shown, import continues
- [ ] Accessibility linter: missing input label → A001 error shown at publish
- [ ] Localization linter: hardcoded text → L001 warning shown at publish
