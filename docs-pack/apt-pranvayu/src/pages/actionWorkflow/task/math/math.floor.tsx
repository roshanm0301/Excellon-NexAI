...
import { ITaskFloor, MathMethodType } from "../../rule/task.math";

export function MathFloor() {
    let { id: stepId, name: stepName, setId, setName, properties, setProperty } = useStepEditor();
    const [formData, setFormData] = useState<ITaskFloor>({
        id: "", // Will be synced from step.id
        type: TaskType.Math, // hardcoded readonly
        method: MathMethodType.Floor, // hardcoded readonly
        name: '',
        expression: '',
  ...
  const { onFieldDataChanged, autoSave } = useAutoSave(formData, stepId, stepName, setProperty, setId, setName);
    ...
    useEffect(() => {
        if (properties?.taskSettings) {
            const data = properties?.taskSettings as ITaskFloor;
            setFormData(prev => ({ ...prev, ...data, id: stepId || data.id || prev.id, method: properties?.type  as MathMethodType.Floor, name: stepName || data.name || '' }));
        } else {
