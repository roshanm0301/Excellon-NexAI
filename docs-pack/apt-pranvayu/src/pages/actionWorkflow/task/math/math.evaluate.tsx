...
import { ITaskEvaluate, MathMethodType } from "../../rule/task.math";

export function MathEvaluate() {
    let { id: stepId, name: stepName, setId, setName, properties, setProperty } = useStepEditor();
    const [formData, setFormData] = useState<ITaskEvaluate>({
        id: "", // Will be synced from step.id
        type: TaskType.Math, // hardcoded readonly
        method: MathMethodType.Evaluate, // hardcoded readonly
        name: '',
        expression: '',
        payload: [],
  ...
  const { onFieldDataChanged, autoSave } = useAutoSave(formData, stepId, stepName, setProperty, setId, setName);
    ...
    useEffect(() => {
        if (properties?.taskSettings) {
            const data = properties?.taskSettings as ITaskEvaluate;
            setFormData(prev => ({ ...prev, ...data, id: stepId || data.id || prev.id, method: properties?.type  as MathMethodType.Evaluate, name: stepName || data.name || '' }));
        } else {
