...
export function JWTVerifySecurity() {
  ...
  useEffect(() => {
    if (properties?.taskSettings) {
      ...
      setFormData(prev => ({ ...prev, ...data, id: stepId || data.id || prev.id, method: properties.type as MethodType.JWTVerify, name: stepName || data.name || '' }));
    } else {
      setFormData(prev => ({ ...prev, id: stepId || prev.id, name: stepName || prev.name }));
    }
  }, [stepId, stepName, properties]);

  const onOptionsCallback = (payload: any) => {
    const _formData = { ...formData, options: payload };
    autoSave(_formData);
  };
              ...
              items: [
                {
                  ...
                  validationRules: [
                    {
                      type: "required",
                      message: isRequiredField("secret"),
                    },
                    // {
                    // 	type: "pattern",
                ...
                {
                  label: { text: "Self Sign", location: "left" },
                  dataField: "selfSign",
