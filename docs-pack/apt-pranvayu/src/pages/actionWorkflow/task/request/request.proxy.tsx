...
import { ITaskRequestProxy, RequestMethodType, TaskType } from "../../rule";
...
export function RequestProxy() {
  ...
  useEffect(() => {
    if (properties?.taskSettings) {
      ...
      setFormData(prev => ({ ...prev, ...data, id: stepId || data.id || prev.id, method: properties.type as RequestMethodType.Proxy, name: stepName || data.name || '' }));
    } else {
  ...
  useEffect(() => {
    if (SchemasDataSource.length > 0) {
      const data = properties?.taskSettings as ITaskRequestProxy;
