...
import { VariableMethodType, ITaskVariableGetById } from "../../rule/task.variable";

export function VariableGet() {
  let { id: stepId, name: stepName, setId, setName, properties, setProperty } =
    useStepEditor();
  const [formData, setFormData] = useState({
    id: "",
    name: "",
    method: VariableMethodType.Get,
  ...
  useEffect(() => {
    if (properties?.taskSettings) {
      const data = properties?.taskSettings as ITaskVariableGetById;
      // Include step's name from useStepEditor (not from taskSettings)
      setFormData(prev => ({ ...prev, ...data, id: stepId || data.id || prev.id, method: properties.type as VariableMethodType.Get, name: stepName || data.name || '' }));
    } else {
    ...
    <>
        ...
        <DXForm onFieldDataChanged={onFieldDataChanged}
          ...
          items={[
            ...
            {
              label: { text: "Variable Id", location: "top" },
              dataField: "variableId",
              validationRules: [
                {
                  type: "required",
                  message: isRequiredField("variableId"),
                },
                // {
                //   type: "pattern",
                //   pattern: regEx.pattern,
                //   message: isValidField(`variableId ${regexEx.pattern}`),
                // },
              ],
            },
            {
