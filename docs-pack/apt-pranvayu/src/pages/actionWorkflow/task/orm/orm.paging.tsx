...
import { ITaskORMPaging, ORMMethodType, TaskType } from "../../rule";

export function PagingORM() {
  ...
  const { onFieldDataChanged, autoSave } = useAutoSave(formData, stepId, stepName, setProperty, setId, setName);
    ...
    useEffect(() => {
        if (properties?.taskSettings) {
            ...
            setFormData(prev => ({ ...prev, ...data, id: stepId || data.id || prev.id, method: properties.type as ORMMethodType.Paging, name: stepName || data.name || '' }));
        } else {
    ...
    useEffect(() => {
        if (SchemasDataSource.length > 0) {
            const data = properties?.taskSettings as ITaskORMPaging;
