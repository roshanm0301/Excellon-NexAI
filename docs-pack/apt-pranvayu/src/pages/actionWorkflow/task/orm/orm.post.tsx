...
import { ITaskPostORM, ORMMethodType, TaskType } from "../../rule";

export function PostORM() {
  ...
  useEffect(() => {
    if (properties?.taskSettings) {
      ...
      setFormData(prev => ({ ...prev, ...data, id: stepId || data.id || prev.id, method: properties.type as ORMMethodType.Post, name: stepName || data.name || '' }));
    } else {
  ...
  useEffect(() => {
    if (SchemasDataSource.length > 0) {
      const data = properties?.taskSettings as ITaskPostORM;
