...
import { ITaskTemplateGetById, TemplateMethodType } from "../../rule/task.template";

export function TemplateGetById() {
  let { id: stepId, name: stepName, setId, setName, properties, setProperty } =
    useStepEditor();
  const [formData, setFormData] = useState({
    id: "",
    name: "",
    method: TemplateMethodType.Get,
  ...
  useEffect(() => {
    if (properties?.taskSettings) {
      const data = properties?.taskSettings as ITaskTemplateGetById;
      setFormData(prev => ({ ...prev, ...data, id: stepId || data.id || prev.id, method: properties.type as TemplateMethodType.Get, name: stepName || data.name || '' }));
    } else {
