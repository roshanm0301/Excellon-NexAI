...
import { addSilverSchemaAPI, getGoldSchemaListAPI, getSchemaAPI, getSilverSchemaAPI, GetWarehouseDatatypesAPI, showNotification, updateSilverSchemaAPI } from '../../redux/actions';
import { useAppDispatch, useAppSelector } from '../../store/customHooks';
import { isRequiredField, isValidField } from '../../utility/utils';
import { AddColumn } from '../schema';
import { SilverSchemaDefinition, validateQueryColumns } from './silverSchema.entity';
import { DXPopup } from '../../components/template';

export const SilverSchemaAddEdit = () => {
  const navigate = useNavigate();
  const { id } = useParams()
  const dispatch = useAppDispatch();
  const [schemaFormData, setSchemaFormData] = useState<any>({
    ...SilverSchemaDefinition,
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (id) {
        const result: any = await dispatch(getSilverSchemaAPI(id));
        if (result) {
  ...
  const createUpdateAPICall = async () => {
    let result: any
    if (id) {
      result = await dispatch(updateSilverSchemaAPI(id, schemaFormData));
    } else {
      result = await dispatch(addSilverSchemaAPI(schemaFormData));
    }
    // if (result?.success) {
    //   navigate("/silver-schema");
    // }
  }
    ...
    <div>
      ...
      >
        ...
        >
          <div>
            <p>
              The column(s) you have entered do not match with the Silver Schema columns:
            </p>
