...
export interface SequentialWorkflowDesignerProps {
	...
	onCustomIconChange: (type: string) => void;
	...
	customIcons?: CustomIconConfiguration
}
...
export function SequentialWorkflowDesigner(props: SequentialWorkflowDesignerProps) {
	...
	const onCustomIconChangeRef = useRef(props.onCustomIconChange);
	...
	const customIcons = props.customIcons;
	...
	function forwardCustomIconChange(type: string) {
		if (designerRef.current) {
			onCustomIconChangeRef.current(type);
		}
	}
	...
	function stepEditorProvider(step: Step, context: StepEditorContext) {
		return editorProvider(
			editorRootRef,
			<StepEditorWrapperContext step={step} context={context} definition={definition.value}>
	...
	useEffect(() => {
		onCustomIconChangeRef.current = props.onCustomIconChange;
	}, [props.onCustomIconChange]);
	...
	useEffect(() => {
		...
		if (designerRef.current) {
			...
			if (isNotChanged) {
				...
				if (customIcons !== undefined && customIcons !== designerRef.current.customIcons()) {
					designerRef.current.setCustomIcons(customIcons);
					// console.log('sqd: customIcons updated');
				}

				if (theme !== undefined) {
					designerRef.current.setTheme(theme);
				}
				return;
			}
		...
		if(customIcons){
			designer.setCustomIcons(customIcons);
		}
		if (isReadonly) {
		...
		designer.onCustomIconChanged.subscribe(type => { forwardCustomIconChange(type) });
	...
	}, [placeholder, definition, selectedStepId, isReadonly, theme, undoStackSize, toolboxConfiguration, stepsConfiguration, extensions, customIcons]);
