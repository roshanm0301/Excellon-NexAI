...
export class Designer {
	...
	public static create(placeholder: HTMLElement, startDefinition: Definition, configuration: DesignerConfiguration): Designer {
		...
		designerContext.state.onCustomIconChanged.subscribe(() => {
			setTimeout(() => designer.onCustomIconChanged.forward(designerContext.state.type));
		});
	...
	/**
	 * @description Fires when the custom Icon has clicked
	 */
	public readonly onCustomIconChanged = new SimpleEvent<string>();
	...
	public setCustomIcons(customIcons: any) {
		this.state.setCustomIcons(customIcons);
	}

	/**
	 * @description Changes the theme. Updates the CSS class on the designer root element.
	 */
