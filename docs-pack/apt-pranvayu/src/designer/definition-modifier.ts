import { createTask } from '../pages/actionWorkflow';
import { wrapDefinition } from '../react';
...
export class DefinitionModifier {
	...
	public tryDelete(stepId: string): boolean {
		...
		this.state.notifyDefinitionChanged(DefinitionChangeType.stepDeleted, result.step._id || result.step.id);

		this.updateDependantFields();
		return true;
	}

	public customIconClicked(type: string): any {
		this.state.notifyCustomIconChanged(type)
		this.state.setCustomIcon(type)
	}
	...
	public tryInsert(step: Step, targetSequence: Sequence, targetIndex: number): boolean {
		...
		this.state.notifyDefinitionChanged(DefinitionChangeType.stepInserted, step._id || step.id);
		this.state.setSelectedStepId(step._id || step.id);
		return true;
	}
	...
	public tryMove(sourceSequence: Sequence, step: Step, targetSequence: Sequence, targetIndex: number): boolean {
		...
		this.state.notifyDefinitionChanged(DefinitionChangeType.stepMoved, step._id || step.id);
		this.state.setSelectedStepId(step._id || step.id);
		return true;
	}
