...
export const ObjectRulePayload = React.memo((props: IPayload) => {
    const { data = [], callback, title, isParent = true, item, config, enableOperator = false, SubscriptionList } = props;
    ...
    const savePayload = (e: any) => {
        ...
        const duplicateExists = payloads?.some(item => item.Key === _payload.Key);

        if (duplicateExists) {
            setErrorMsg("A record with the same Key already exists.");
            return;
        }
    ...
    const onCallBack = (callbackData: any[]) => {
        ...
        // If no callback data exists, filter the payloads to remove "Condition" types, and update the state and callback.
        if (!callbackData || callbackData.length === 0) {
            const filteredPayloads = payloads?.filter((payload) => payload.Type !== "Condition") || [];
            setPayloads(filteredPayloads);
            callback(filteredPayloads);
            return;
        }
    
        // Helper function to update existing payloads with new data.
        const updatePayloads = (existingPayloads: any[], newPayloads: any[]) => {
            const newIds = new Set(newPayloads.map(item => item.id)); // Set for faster lookup.
            ...
            const updatedPayloads = existingPayloads.map(existingPayload => {
                if (newIds.has(existingPayload.id)) {
                    ...
                    return { ...existingPayload, ...newData }; // Merge new data with the existing payload.
                }
                return existingPayload;
            });
    
            // Add new payloads that don't exist in the current list.
            newPayloads.forEach(newData => {
                if (!existingPayloads.some(existing => existing.id === newData.id)) {
                    updatedPayloads.push(newData);
                }
            });
        ...
        // Get the current state of the payloads and update them with the new callback data.
        const currentPayloads = payloads || [];
        const updatedPayloads = updatePayloads(currentPayloads, callbackData);
    
        // Update the state with the modified payloads and call the callback function.
                        ...
                        {payloads?.filter((i: any) => i.Type !== "Condition")?.length > 0 && (
                            <div style={{marginTop:'5px'}}>
                                <DataGrid
                                    ...
                                    dataSource={payloads?.filter((i: any) => i?.Type !== "Condition")}
                                    keyExpr={"id"}
                                    columns={gridColumns}
